...
class SolrServiceImplTest extends FunSuite with BaseSolrCloudTest {
  ...
  test("Documents are added to solr collection") {
    ...
    val response = solrService.query(collectionName, "level: ERROR")
  ...
  test("Insert properties dynamic fields from mdc") {
    ...
    val response = solrService.query(collectionName, "hostname: host1.com")
